  ...
  describe '.call' do
    context 'when successful' do
      it 'creates a new project on the target cluster with correct attributes' do
        result = described_class.call(source_project:, target_cluster:)

        expect(result).to be_success
        migrated = result.project
        ...
        expect(migrated.repository_url).to eq(source_project.repository_url)
        expect(migrated.branch).to eq(source_project.branch)
      end

      it 'creates credential provider and configurations' do
        result = described_class.call(source_project:, target_cluster:)
        migrated = result.project
        ...
        expect(migrated.project_credential_provider.provider_id).to eq(source_project.project_credential_provider.provider_id)
      ...
      it 'creates services with resource constraints and domains' do
        service = create(:service, project: source_project)
        create(:resource_constraint, service:)
        create(:domain, service:)

        result = described_class.call(source_project: source_project.reload, target_cluster:)
        migrated = result.project
      ...
      it 'creates environment variables, volumes, and notifiers' do
      ...
      it 'stores canine_config on the migrated project' do
        create(:service, project: source_project)
        create(:environment_variable, project: source_project)

        result = described_class.call(source_project: source_project.reload, target_cluster:)
        migrated = result.project

        expect(migrated.canine_config).to be_present
        expect(migrated.canine_config['project']).to be_present
        expect(migrated.canine_config['services']).to be_present
        expect(migrated.canine_config['environment_variables']).to be_present
      end
    end
    ...
    context 'when restore fails' do
      it 'returns failure context' do
        ...
        expect(result.message).to include("Failed to restore project")
      end
    end
  end
end
