...
RSpec.describe Networks::CheckDns do
  ...
  let(:gateway) { K8::Stateless::Gateway.new(service).connect(connection) }

  describe 'Dns::Utils.infer_expected_hostname' do
    context 'when gateway returns public IP' do
      before do
        allow(gateway).to receive(:hostname).and_return({ value: '8.8.8.8', type: :ip_address })
      end

      it 'returns the IP' do
        expect(Dns::Utils.infer_expected_hostname(gateway, connection)).to eq({ value: "8.8.8.8", type: :ip_address })
      end
    end

    context 'when gateway returns private IP' do
      before do
        allow(gateway).to receive(:hostname).and_return({ value: '10.0.0.1', type: :ip_address })
      ...
      context 'when cluster is a single node cluster' do
        it 'resolves and returns public IP' do
          expect(Dns::Utils.infer_expected_hostname(gateway, connection)).to eq({ value: "1.2.3.4", type: :ip_address })
        end
      end
      context 'when cluster is not a single node cluster' do
        let(:cluster) { create(:cluster, cluster_type: :k8s) }
        it 'raises an error' do
          expect { Dns::Utils.infer_expected_hostname(gateway, connection) }.to raise_error("Private IP address detected for cluster type: k8s")
        end
      end
    end
    ...
    context 'when server hostname is an IP' do
      ...
      before do
        allow(gateway).to receive(:hostname).and_return({ value: '1.2.3.4', type: :ip_address })
      end
