...
RSpec.describe AddOns::FetchChartDetailsFromRepositoryUrl do
  let(:repository_url) { "https://serhanekicii.github.io/openclaw-helm" }
  let(:index_url) { "#{repository_url}/index.yaml" }
  ...
  describe "successful fetch" do
    ...
    it "fetches and parses repository index" do
      result = described_class.execute(repository_url: repository_url)
    ...
    it "handles trailing slash in repository_url" do
      stub_request(:get, "#{repository_url}/index.yaml")
        .to_return(status: 200, body: index_yaml, headers: { "Content-Type" => "text/yaml" })
